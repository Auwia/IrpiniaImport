        ...
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (item.id != null && item.title != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item.id, item.title, item.price, item.quantity);
...
shoppingCart.prototype.addItem = function (id, title, price, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(id, title, price, quantity);
            this.items.push(item);
        }
...
shoppingCart.prototype.checkoutGoogle = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        ...
        data["item_description_" + ctr] = item.title;
